...
from s2s.aurora.model.film import AdaptiveLayerNorm
from s2s.aurora.model.fourier import lead_time_expansion
from s2s.aurora.model.lora import LoRAMode, LoRARollout
from s2s.aurora.model.util import init_weights, maybe_adjust_windows
...
class Swin3DTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        window_size: Union[int, Tuple[int, int, int]] = 7,
        mlp_ratio: float = 4.0,
        qkv_bias: bool = True,
        drop_rate: float = 0.0,
        ...
        Args:
            ...
            window_size (Union[int, Tuple[int, int, int]]): Window size. Defaults to `7`.
            mlp_ratio (float): Hidden layer dimensionality divided by that of the input for all
                MLPs. Defaults to `4.0`.
            qkv_bias (bool): If `True`, add a learnable bias to the query, key, and value. Defaults
                to `True`.
