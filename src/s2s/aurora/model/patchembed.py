...
__all__ = ["LevelPatchEmbed", "VariablePatchEmbed"]
...
class VariablePatchEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor, var_names: Tuple[str, ...]) -> torch.Tensor:
        ...
        # Select the biases of the variables that are present in the batch.
        biases = torch.cat(
            [
                self.bias[name]
                for name in var_names
        ...
        proj = F.conv2d(
                input=x,        # (B, V, H, W)
                weight=weights, # (V*D, 1, P, P)
                bias=biases,    # (V*D)
                stride=self.kernel_size,
                groups=V
                )
        
        proj = proj.view(B,V,self.embed_dim, proj.shape[-2], proj.shape[-1]) #(B, V, D, H/P, W/P)
        
        if self.flatten:
            proj = proj.reshape(B, V, self.embed_dim, -1)  # (B, V, D, L)
            proj = proj.transpose(2, 3)  # (B, V, L, D)
