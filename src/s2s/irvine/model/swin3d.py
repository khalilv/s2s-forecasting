...
from s2s.irvine.model.film import AdaptiveLayerNorm
from s2s.irvine.model.fourier import lead_time_expansion
from s2s.irvine.model.util import init_weights, maybe_adjust_windows
    ...
    def __init__(
        ...
        Args:
            ...
            scale_bias (float, optional): Scale bias for
                :class:`irvine.model.film.AdaptiveLayerNorm`. Defaults to `0`.
        """
        super().__init__()
        self.dim = dim
...
class BasicLayer3D(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            scale_bias (float, optional): Scale bias for
                :class:`irvine.model.film.AdaptiveLayerNorm`. Default: 0
        """
        super().__init__()
        ...
        if downsample is not None:
            self.downsample: Optional[PatchMerging3D] = downsample(dim=dim)
        else:
            self.downsample = None

        if upsample is not None:
            self.upsample: Optional[PatchSplitting3D] = upsample(dim=dim)
        else:
            self.upsample = None
...
class Swin3DTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        window_size: Union[int, Tuple[int, int, int]] = (2, 6, 12),
        mlp_ratio: float = 4.0,
        qkv_bias: bool = True,
        drop_rate: float = 0.0,
        ...
        """
        Args:
            embed_dim (int): Patch embedding dimension. Defaults to `512`.
            ...
            decoder_num_heads (Tuple[int, ...]): Number of attention heads in each decoder layer.
                Defaults to `(16, 8, 4)`.
            window_size (Union[int, Tuple[int, int, int]]): Window size. Defaults to `(2, 6, 12)`.
            mlp_ratio (float): Hidden layer dimensionality divided by that of the input for all
                MLPs. Defaults to `4.0`.
            qkv_bias (bool): If `True`, add a learnable bias to the query, key, and value. Defaults
                to `True`.
